...
<script>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data[0].forEach(element => {
                    var valid = true
                    if (element != null) {
                        if (element.sub_kriteria != null) {
                            element.sub_kriteria.forEach(element => {
                                if (element.bobot_mahasiswa != null) {
                                    element.bobot_mahasiswa.forEach(element => {
                                        if (element.mahasiswa_id ==
                                            idMahasiswa) {
                                            valid = false
                                        }
                                    })
            ...
            success: function(data) {
                ...
                data[0].forEach(element => {
                    if (element.id == idKriteria) {
                        namaKriteria = element.nama
                        if (element.sub_kriteria !== null) {
                            element.sub_kriteria.forEach(element => {
                                var valid = true
                                element.bobot_mahasiswa.forEach(element => {
                                    // console.log(element)
                                    if (idMahasiswa == element
                                        .mahasiswa_id) {
                                        valid = false
                                    }
                                })
                ...
                $("#sub_kriteria_id").html(
                    '<option value="" selected disabled>Pilih Sub Kriteria ' + namaKriteria +
                    '</option>' + html
                );
            }
        })
    ...
    @elseif (Session::has('fatalErrMessage'))
        Swal.fire({
            icon: 'error',
            title: '{{ Session::get('errMessage') }}'
            // timer: 3000
